...
Modules: `auth`, `users`, `consultations`, `conversations`, `chat`, `requirement-summary`, `feature-detection`, `estimation`, `proposal`, `feature-library`, `client-lead`, `lead-proposal`, the `client-*` Client Portal modules, plus non-domain `ai/` (provider abstraction) and `prompts/` (templating). Empty scaffolds (`.gitkeep` only): `admin`, `crm`, `dashboard`.

...
- **The planner also decides the art direction, and `imagePrompt` is the only thing that carries it.** No other planner field reaches the image model, so the `CONCEPT_SCREENS` prompt derives a per-project palette/typography/mood from the client's industry (colour psychology, not chance) and requires it restated **verbatim in every screen's `imagePrompt`** — each screen is a separate image call with no memory of the others, so a drifting palette stops the set looking like one product. `IMAGE_STYLE_DIRECTIVE` (`client-mockups.service.ts`) is the other half of that one prompt and the two must be edited together: it deliberately **names no colours of its own** and instead declares the art direction above it binding, because a house palette there would make every project identical — which is exactly what its old "flat, modern, professional colour palette" line was doing.

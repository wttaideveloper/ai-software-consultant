...
const SYSTEM_PROMPT_TEMPLATES = {
    ...
    [prompt_constants_js_1.PROMPT_TYPES.FEATURE_DETECTION]: "You are a product analyst for {{organizationName}}. Extract the work items from the requirement summary for \"{{consultationTitle}}\". Industry: {{industry}}. Project type: {{projectType}}. ENGAGEMENT TYPE: {{modeLabel}}. {{modeContext}} {{modeFeatureDirective}} Every item's `category` MUST be chosen from exactly this list, which is specific to this engagement type: {{modeFeatureCategories}}. Never use a category outside that list, and never carry over the category vocabulary of a different engagement type. Respond with ONLY valid JSON (no markdown fences) using this exact shape: {\"features\":[{\"name\":\"\",\"category\":\"\",\"description\":\"\",\"priority\":\"HIGH|MEDIUM|LOW\",\"complexity\":\"LOW|MEDIUM|HIGH\",\"confidence\":0.0,\"reasoning\":\"\"}]}. confidence must be between 0 and 1. Do not invent unsupported items.",
    ...
    [prompt_constants_js_1.PROMPT_TYPES.REQUIREMENT_SUMMARY]: "You are a requirements analyst for {{organizationName}}. Analyze the consultation conversation for \"{{consultationTitle}}\". Industry: {{industry}}. Project type: {{projectType}}. ENGAGEMENT TYPE: {{modeLabel}}. {{modeContext}} {{modeSummaryDirective}} Populate the fields below in the terms this engagement type actually uses — for a support or migration engagement, `coreFeatures` holds the work items in scope rather than product features, and `targetUsers` describes who is affected rather than who a new product is for. Respond with ONLY valid JSON (no markdown fences) using this exact shape: {\"summaryMarkdown\":\"string\",\"structuredSummary\":{\"projectName\":\"\",\"projectType\":\"\",\"businessGoals\":[],\"targetUsers\":[],\"coreFeatures\":[],\"adminFeatures\":[],\"integrations\":[],\"nonFunctionalRequirements\":[],\"assumptions\":[],\"openQuestions\":[]}}. summaryMarkdown must be a complete markdown requirements document. Do not invent facts not supported by the conversation.",
    [prompt_constants_js_1.PROMPT_TYPES.ESTIMATION]: "You are a software estimation specialist for {{organizationName}}. Produce effort estimates for \"{{consultationTitle}}\". Industry: {{industry}}. Project type: {{projectType}}. ENGAGEMENT TYPE: {{modeLabel}}. {{modeContext}} {{modeEstimationDirective}} Derive the effort entirely from the requirements: the client is never asked for a duration or a budget, so there is no client-stated timeline or budget to honour and you must never assume one. Respond with ONLY valid JSON (no markdown fences) using this exact shape: {\"estimatedHours\":0,\"estimatedWeeks\":0,\"teamSize\":0,\"complexity\":\"LOW|MEDIUM|HIGH\",\"confidence\":0.0,\"assumptions\":[],\"risks\":[],\"breakdown\":[{\"category\":\"\",\"hours\":0}],\"techStack\":[],\"maintenancePlan\":null,\"migrationPlan\":null,\"enhancementImpact\":null}. confidence must be between 0 and 1. estimatedHours and breakdown are always required, and the breakdown's hours must sum to estimatedHours. techStack must list recommended technologies — never a cost or price. {{techStackBaseline}} Whatever you return there, for this engagement type: {{modeTechStackDirective}}. The last three keys are engagement-specific: fill in exactly the ONE that matches this engagement type and leave the other two null. For Build a New Project fill none of them. For Enhance an Existing Project set enhancementImpact to {\"impactAnalysis\":[\"\"],\"dependencies\":[\"\"],\"affectedModules\":[\"\"]}. For Modernization & Migration set migrationPlan to {\"currentStack\":[],\"phases\":[{\"name\":\"\",\"description\":\"\",\"hours\":0}],\"rollbackStrategy\":\"\",\"downtimeEstimate\":\"\"}, where currentStack names the technologies being migrated away from (the client's existing stack, as stated in the requirements — never invented) so the client can be shown a current-versus-recommended comparison with the phases in the order they must be executed and their hours summing to estimatedHours. For Maintenance & Support set estimatedWeeks to null — a support engagement has no delivery date and inventing one is wrong — and set maintenancePlan to {\"engagementType\":\"ONE_TIME_FIX|MONTHLY_RETAINER|ONGOING_SUPPORT\",\"supportHoursPerMonth\":0,\"priorityLevel\":\"LOW|MEDIUM|HIGH|CRITICAL\",\"suggestedSla\":\"\",\"supportScope\":[\"\"]}, where supportHoursPerMonth is the recurring monthly capacity the engagement needs and suggestedSla states response and resolution targets in plain words. For every engagement type other than Maintenance & Support, estimatedWeeks must be a positive number. Base estimates only on the provided requirement summary and detected work items.",
    [prompt_constants_js_1.PROMPT_TYPES.PROPOSAL]: "You are a proposal writer for {{organizationName}}. Draft a client-ready \"{{modeProposalTitle}}\" for \"{{consultationTitle}}\". Industry: {{industry}}. Project type: {{projectType}}. ENGAGEMENT TYPE: {{modeLabel}}. {{modeContext}} {{modeProposalDirective}} The document must read as a {{modeProposalTitle}} throughout — its `title` must name it as one, and it must never be written as a generic new-build project proposal when the engagement is something else. The timeline you state must come from the supplied estimation, never from anything the client said and never invented; the client was never asked for a budget or a date, so never refer to a client budget, a client deadline, or a launch date they gave. When the supplied estimation has no delivery weeks (a support engagement), the `timeline` field must describe the engagement model and its review cadence instead of a delivery schedule, and must contain no completion date. Respond with ONLY valid JSON (no markdown fences) using this exact shape: {\"title\":\"\",\"executiveSummary\":\"\",\"scopeOfWork\":[],\"deliverables\":[],\"timeline\":\"\",\"assumptions\":[],\"exclusions\":[],\"pricingNotes\":\"\",\"proposalMarkdown\":\"\"}. proposalMarkdown must never be empty: it is a complete, standalone client-facing markdown document (with headings) that restates the title, executive summary, scope of work, deliverables, timeline, assumptions, exclusions, and pricing notes in full prose. assumptions and exclusions must each contain at least one item; if none are notable, include a single explicit entry such as \"No additional assumptions beyond the requirement summary.\" or \"No exclusions beyond the defined scope of work.\". Base the proposal only on the provided requirement summary, detected features, and estimation. Do not invent unsupported scope.",
    ...
    [prompt_constants_js_1.PROMPT_TYPES.CONCEPT_SCREENS]: [
        ...
        "STEP 4 — SCREENS. ENGAGEMENT TYPE: {{modeLabel}}. {{modeContext}} If this engagement adds functionality to an application that already exists, every screen must depict the NEW functionality as it would appear inside that existing product — never a from-scratch onboarding or sign-up flow the client already has. If it is a migration that includes a UI redesign, the screens depict the redesigned equivalents of screens the client already runs. Choose exactly {{screenCount}} screens that best represent this specific project's core user journey, in the order a real user would encounter them. The screens MUST be specific to this project's domain — a food delivery app should yield screens like Login, Home, Restaurant Details, Cart, Checkout, while a CRM should yield Login, Dashboard, Customers, Reports, Settings. Never return generic placeholder screens when the requirements describe a specific domain. Target platforms: {{platforms}}.",
    ...
    [prompt_constants_js_1.PROMPT_TYPES.CLIENT_REQUIREMENT_DISCOVERY]: "You are a friendly software consultant conducting a brief pre-consultation discovery interview with a prospective client, before they meet a human consultant. ENGAGEMENT TYPE: {{modeLabel}}. {{modeContext}} {{modeDiscoveryDirective}} Every question you ask must serve this engagement type — cover, in roughly this order: {{modeDiscoveryFocus}}. Asking a question that belongs to a different engagement type (for example asking a maintenance client for their business idea and target users, or asking a new-build client what their current stack is) is the single worst mistake you can make here. The client describes their need as: \"{{projectIdea}}\". They have chosen a {{consultationTime}}-minute consultation, which is exactly {{totalQuestions}} questions long — that length is the client's own choice and it is fixed, so ask all {{totalQuestions}} of them, no more and no fewer. Target platforms: {{platforms}}. You have already asked {{questionsAskedSoFar}} of those {{totalQuestions}} questions, so {{questionsRemaining}} remain. This is a hard limit in BOTH directions, not a suggestion. If {{questionsAskedSoFar}} is already at or above {{totalQuestions}}, you MUST set completed to true and question to null right now, even if you can think of another good question — never exceed the total. While {{questionsRemaining}} is greater than zero you MUST set completed to false and ask the next question — never end the interview early, even if you feel you already have a clear picture, and never combine topics to finish sooner. Ask exactly ONE short, clear, plain-language question at a time — no markdown, no numbering, no preamble, no multiple questions in one turn. Build on the client's previous answers rather than repeating a topic already covered. The purpose of this conversation is to understand the client's business requirements and project needs — what they want to build, not what it should cost or when it should ship. Do not ask about budget, cost, pricing, spend, timeline, launch date, delivery date, target completion date, or project duration, in any wording; these are calculated later by the system from the requirements you gather, and every one of your {{totalQuestions}} questions must go to a requirement instead. If the client volunteers a budget or a date, accept it without comment and ask your next requirement question. Plan the whole interview to fit the length you have been given, working down the engagement type's topic list above: with few questions, cover only the first and most essential of those topics; with more questions available, spend the extra ones going deeper into the later topics on that same list — depth that is genuinely useful to the human consultant, never filler, and never a topic from a different engagement type. Output ONLY a single syntactically valid JSON object and nothing else: no markdown fences, no comments, no text before or after it, and no extra or missing braces. Use exactly this shape: {\"question\":\"string or null\",\"completed\":true|false}. When completed is true, question must be null. When completed is false, question must be a non-empty question and completed must be false.",
};
...
class PromptBuilder {
    ...
    /**
     * Expands one consultation mode into the full set of `{{mode*}}` variables from
     * its profile.
     *
     * This is the join between the mode registry and every prompt. Callers pass a
     * single enum value; the eleven directive strings are looked up here, so no
     * service ever hand-assembles mode wording and the four engagement types stay
     * defined in exactly one file.
     */
    injectConsultationModeContext(mode, variables = {}) {
        const resolved = (0, consultation_mode_js_1.normalizeConsultationMode)(mode);
        const profile = (0, consultation_mode_profiles_js_1.getConsultationModeProfile)(resolved);
        return {
            ...variables,
            [prompt_constants_js_1.RESERVED_TEMPLATE_VARIABLES.CONSULTATION_MODE]: resolved,
            [prompt_constants_js_1.RESERVED_TEMPLATE_VARIABLES.MODE_LABEL]: profile.label,
            [prompt_constants_js_1.RESERVED_TEMPLATE_VARIABLES.MODE_CONTEXT]: profile.context,
            [prompt_constants_js_1.RESERVED_TEMPLATE_VARIABLES.MODE_DISCOVERY_FOCUS]: profile.discoveryFocus,
            [prompt_constants_js_1.RESERVED_TEMPLATE_VARIABLES.MODE_DISCOVERY_DIRECTIVE]: profile.discoveryDirective,
            [prompt_constants_js_1.RESERVED_TEMPLATE_VARIABLES.MODE_SUMMARY_DIRECTIVE]: profile.summaryDirective,
            [prompt_constants_js_1.RESERVED_TEMPLATE_VARIABLES.MODE_FEATURE_CATEGORIES]: profile.featureCategories,
            [prompt_constants_js_1.RESERVED_TEMPLATE_VARIABLES.MODE_FEATURE_DIRECTIVE]: profile.featureDirective,
            [prompt_constants_js_1.RESERVED_TEMPLATE_VARIABLES.MODE_ESTIMATION_DIRECTIVE]: profile.estimationDirective,
            [prompt_constants_js_1.RESERVED_TEMPLATE_VARIABLES.MODE_TECH_STACK_DIRECTIVE]: profile.techStackDirective,
            [prompt_constants_js_1.RESERVED_TEMPLATE_VARIABLES.MODE_PROPOSAL_TITLE]: profile.proposalTitle,
            [prompt_constants_js_1.RESERVED_TEMPLATE_VARIABLES.MODE_PROPOSAL_DIRECTIVE]: profile.proposalDirective,
        };
    }
    ...
    build(input) {
        ...
        // An explicit consultationMode wins over the consultation record's own: the
        // stateless Client Portal has no record to read one from.
        variables = this.injectConsultationModeContext(input.consultationMode ?? input.consultation?.consultationMode, variables);
        variables = {
            ...variables,

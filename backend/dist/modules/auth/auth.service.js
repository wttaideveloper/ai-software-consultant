...
class AuthService {
     ...
     */
    async refreshSession(input, context) {
        const invalidSession = () => new app_error_js_1.AppError("Session expired. Please log in again.", http_status_js_1.HTTP_STATUS.UNAUTHORIZED);
        let claims;
        try {
            claims = (0, jwt_js_1.verifyRefreshToken)(input.refreshToken);
        }
        catch {
        ...
        const stored = await auth_repository_js_1.authRepository.findRefreshTokenByHash(hashRefreshToken(input.refreshToken));
        ...
        const record = await auth_repository_js_1.authRepository.findUserWithOrganizationById(claims.sub);
        ...
        const accessToken = (0, jwt_js_1.generateAccessToken)(tokenInput);
        const refreshToken = (0, jwt_js_1.generateRefreshToken)(tokenInput);
        await auth_repository_js_1.authRepository.runInTransaction(async (tx) => {
            await auth_repository_js_1.authRepository.deleteRefreshTokensByUserId(record.user.id, tx);
            await auth_repository_js_1.authRepository.createRefreshToken({
        ...
        logger_js_1.logger.debug(`Session refreshed for user: ${record.user.email}`);
        return {

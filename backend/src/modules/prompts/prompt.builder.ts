...
import { getConsultationModeProfile } from "./consultation-mode.profiles.js";
import {
  PROMPT_DEFAULTS,
  PROMPT_TYPES,
...
export class PromptBuilder {
   ...
   */
  injectConsultationModeContext(
    mode: unknown,
    variables: TemplateVariables = {},
  ): TemplateVariables {
    const resolved = normalizeConsultationMode(mode);
    const profile = getConsultationModeProfile(resolved);

    return {
      ...variables,
      [RESERVED_TEMPLATE_VARIABLES.CONSULTATION_MODE]: resolved,
      [RESERVED_TEMPLATE_VARIABLES.MODE_LABEL]: profile.label,
      [RESERVED_TEMPLATE_VARIABLES.MODE_CONTEXT]: profile.context,
      [RESERVED_TEMPLATE_VARIABLES.MODE_DISCOVERY_FOCUS]: profile.discoveryFocus,
      [RESERVED_TEMPLATE_VARIABLES.MODE_DISCOVERY_DIRECTIVE]:
        profile.discoveryDirective,
      [RESERVED_TEMPLATE_VARIABLES.MODE_SUMMARY_DIRECTIVE]:
        profile.summaryDirective,
      [RESERVED_TEMPLATE_VARIABLES.MODE_FEATURE_CATEGORIES]:
        profile.featureCategories,
      [RESERVED_TEMPLATE_VARIABLES.MODE_FEATURE_DIRECTIVE]:
        profile.featureDirective,
      [RESERVED_TEMPLATE_VARIABLES.MODE_ESTIMATION_DIRECTIVE]:
        profile.estimationDirective,
      [RESERVED_TEMPLATE_VARIABLES.MODE_TECH_STACK_DIRECTIVE]:
        profile.techStackDirective,
      [RESERVED_TEMPLATE_VARIABLES.MODE_PROPOSAL_TITLE]: profile.proposalTitle,
      [RESERVED_TEMPLATE_VARIABLES.MODE_PROPOSAL_DIRECTIVE]:
        profile.proposalDirective,
    };
  }
  ...
  build(input: PromptBuildInput): BuiltPrompt {
    ...
    variables = this.injectConsultationModeContext(
      input.consultationMode ?? input.consultation?.consultationMode,
      variables,
    );
    variables = {
      ...variables,
